...
def style_transfer_main(content, styles, output, iterations):
    parse = build_parser()
    ...
    if not os.path.isfile(options.content):
        parse.error("Content %s does not exist" % options.content)
    
    #if not os.path.isfile(options.styles):
    #    parse.error("Style %s does not exist" % options.styles)

    if not os.path.isfile(options.network):
        parse.error("Network %s does not exist. (Did you forget to download it?)" % options.network)
